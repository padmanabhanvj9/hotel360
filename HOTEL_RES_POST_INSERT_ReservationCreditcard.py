...
def Hotel_RES_Post_Update_UpdateReservationCreditcard(request):
    d = request.json
    print(d)
    a = { k : v for k,v in d.items() if v != '' if k not in ('res_id','pf_id','cc_id')}
    print(a)
    e = { k : v for k,v in d.items() if k != '' if k in ('res_id','pf_id','cc_id')}
    print(e)
...
def Hotel_RES_POST_Delete_DeleteReservationCreditcard(request):
     ...
     cc_id = str(cc_id)[1:-1]
     sql_value = dbput("delete from profile.pf_creditcard where res_id = '"+res_id+"' and pf_id = '"+pf_id+"' and cc_id in ("+cc_id+")")
     print(sql_value)
