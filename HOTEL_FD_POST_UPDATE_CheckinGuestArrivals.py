...
def HOTEL_FD_POST_UPDATE_CheckinGuestArrivals(request):
   ...
   arrival = dbget("select res_arrival, res_adults,res_room from reservation.res_reservation where res_id = '"+res_id+"' and pf_id = '"+pf_id+"'")
   ...
   if RES_Log_Date == today_arrival:
       ...
       if len(sql_value) != 0:
          psql = dbput("delete from room_management.rm_queue_room where res_id = '"+res_id+"'")
          print(psql)
       else:
          pass
       e = {}
